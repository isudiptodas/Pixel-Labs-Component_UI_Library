    ...
    <>
      <div className={`w-full md:w-[60%] py-4 px-3 h-auto relative overflow-hidden flex flex-col justify-center items-center`}>
        ...
        <motion.div
          ...
          className={`w-full max-h-[40vh] overflow-y-auto flex flex-col justify-start items-center gap-2 bg-gray-100 mt-1`}>
          {optionList.map((item, index) => {
            return <div onClick={() => { handleSelectOption(item.name); setDropdownVisible(false); }} key={index} className={`w-full group cursor-pointer flex flex-col justify-center items-start px-3 py-2 hover:bg-white hover:font-semibold duration-500 ease-in-out`}>
              <p className={`w-full text-sm text-start capitalize text-black pb-2 border-b border-gray-300`}>{item.name}</p>
              <p className={`w-full text-[12px] text-start capitalize text-black py-2 opacity-70 italic`}>{item.description}</p>
            </div>
          })}
        </motion.div>
      </div>
