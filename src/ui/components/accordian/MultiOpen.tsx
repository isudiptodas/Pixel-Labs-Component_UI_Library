            ...
            <div className={`w-full h-auto flex flex-col justify-start items-center gap-5 py-2 px-3`}>
                {data.map((item, index) => {
                    ...
                    return <div key={index} className={`w-full h-auto border border-gray-400 rounded-xl flex flex-col justify-start items-center bg-white overflow-hidden`}>
                        ...
                        <div onClick={() => handleAdd(index)} className={`w-full font-semibold ${isOpen ? "shadow-md" : ""} cursor-pointer flex justify-between items-center px-5 py-3`}>
                            <p className={`text-black text-sm lg:text-lg`}>{item.title}</p>
                            <span className={`text-sm text-black`}><FaPlus className={`${isOpen ? "rotate-45" : "rotate-0"} duration-200 ease-in-out`} /></span>
                        </div>
                        <AnimatePresence>
                            <motion.div
                                initial={false}
                                animate={{
                                    height: isOpen ? bounds.height : '0px',
                                    opacity: isOpen ? 1 : 0,
                                    paddingBottom: isOpen ? '60px' : "0px",
